...
class AsyncReadRepository(AsyncBaseRepository, Generic[T]):
    """
    A base class for implementing an async read Repository or DAO for performing read operations.

    ```python
    job_dao = AsyncReadRepository(model=Job, session=async_session)
