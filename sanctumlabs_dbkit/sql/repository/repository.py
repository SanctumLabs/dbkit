...
class BaseRepository(Generic[T], metaclass=ABCMeta):
    """
    A base class for implementing a Repository or DAO. This
    ...
    def _supports_soft_deletion(model: Type[T]) -> TypeGuard[Type[AbstractBaseModel]]:
        """
        Indicates if the provided model supports soft deletion (has a 'deleted_at' column). This function
        takes in an argument due to mypy type guarding requirements, and is thus static.
        """
        return issubclass(model, AbstractBaseModel)
...
class WriteRepository(BaseRepository, Generic[T]):
    """
    A base class for implementing write operations on a Repository or DAO.

    ```python
    job_dao = WriteRepository(model=Job, session=session)

    job = job_dao.create("123")
    """

    def __init__(self, model: Type[T], session: Session) -> None:
        """Creates an instance of the WriteRepository"""
...
class ReadRepository(BaseRepository, Generic[T]):
    """
    A base class for implementing read operations on a Repository or DAO.
    ...
    def __init__(self, model: Type[T], session: Session) -> None:
        """Creates an instance of the ReadRepository"""
        super().__init__(model, session)
        self.model = model
        self.session = session
